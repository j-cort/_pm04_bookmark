...
class Bookmarks
  ...
  def delete_button
    "<form class='#{@title} bm-delete-button' action='/delete_bookmark' method='post'>
      ...
      <button type='submit' value='Delete'><i class='fas fa-trash-alt fa-2x'></i></button>
    </form>"    
  end

  def edit_button
    "<div class='#{@title} bm-edit-button'>
      <button type='submit' value='Edit' id='edit-button-#{@id}' onclick='toggleEditForm(#{@id})'><i class='fas fa-edit fa-2x'></i></button>
    </div>" 
  end
  ...
  def comments_button
    "<div class='#{@title} bm-comments-button'>
      <button type='submit' value='Comments' id='comments-button-#{@id}' onclick='toggleComments(#{@id})'><i class='far fa-comment fa-2x'></i></button>
    </div>" 
  end
  ...
  def tags_button
    "<div class='#{@title} bm-tags-button'>
      <button type='submit' value='Tags' id='tags-button-#{@id}' onclick='toggleTags(#{@id})'><i class='far fa-sticky-note fa-2x'></i></button>
    </div>" 
  end
